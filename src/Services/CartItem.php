...
namespace Webtamizhan\StatelessCart\Services;
...
class CartItem implements Arrayable, Jsonable
{
    ...
    {
        if (is_null($decimals)) {
            $decimals = is_null(config('stateless-cart.format.decimals')) ? 2 : config('stateless-cart.format.decimals');
        }

        if (is_null($decimalPoint)) {
            $decimalPoint = is_null(config('stateless-cart.format.decimal_point')) ? '.' : config('stateless-cart.format.decimal_point');
        }

        if (is_null($thousandSeperator)) {
            $thousandSeperator = is_null(config('stateless-cart.format.thousand_seperator')) ? ',' : config('stateless-cart.format.thousand_seperator');
        }
